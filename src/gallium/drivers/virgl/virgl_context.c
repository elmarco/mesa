...
{
   struct virgl_context *vctx = (struct virgl_context *)ctx;
   virgl_encoder_set_viewport_state(vctx, state);
}
...
//      util_primconvert_save_rasterizer_state(vctx->primconvert, vctx->rasterizer);
...
//           virgl_hw_set_index_buffer(ctx, NULL);
...
{
   struct virgl_context *vctx = (struct virgl_context *)ctx;
   virgl_encoder_set_scissor_state(vctx, ss);
}
                                                         ...
                                                         void *priv)
{
   ...
   vctx->blitter = NULL;//util_blitter_create(&vctx->base);
